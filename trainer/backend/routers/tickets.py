...
def create_ticket(ticket: TicketCreate, current_user: dict = Depends(get_current_user)):
    my_role = current_user.get("role"); my_id = current_user.get("id")
    subject = ticket.subject.strip(); initial_message = ticket.initial_message.strip()
    ...
    try:
        ...
        cursor.execute("INSERT INTO support_tickets (subject, status, initiator_id, initiator_role, receiver_id, receiver_role) VALUES (%s, 'Open', %s, %s, %s, %s)", (subject, my_id, my_role, ticket.receiver_id, ticket.receiver_role))
        ticket_id = cursor.lastrowid
        cursor.execute("INSERT INTO support_ticket_messages (ticket_id, sender_id, message_text) VALUES (%s, %s, %s)", (ticket_id, my_id, initial_message))
        db.commit()
        return {"ticket_id": ticket_id, "message": "Ticket created successfully."}
    except HTTPException:
        db.rollback(); raise
    except Exception:
        db.rollback(); raise HTTPException(status_code=500, detail="Internal server error")
    finally:
        cursor.close(); db.close()
    ...
    try:
        cursor.execute(f"""
            SELECT t.id, t.subject, t.status, t.created_at, t.updated_at,
                   t.initiator_id, t.initiator_role, {_display_name_sql('u1')} AS initiator_name,
                   t.receiver_id, t.receiver_role, {_display_name_sql('u2')} AS receiver_name
    ...
    try:
        ...
        cursor.execute(f"""
            SELECT m.id, m.sender_id, m.message_text, m.created_at,
                   {_display_name_sql('u')} AS sender_name, u.role AS sender_role
...
def reply_ticket(ticket_id: int, message: MessageCreate, current_user: dict = Depends(get_current_user)):
    my_id = current_user.get("id"); message_text = message.message_text.strip()
    if not message_text:
    ...
    try:
        ...
        cursor.execute("INSERT INTO support_ticket_messages (ticket_id, sender_id, message_text) VALUES (%s, %s, %s)", (ticket_id, my_id, message_text))
