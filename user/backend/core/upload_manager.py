...
async def save_upload_file(file: UploadFile, category: str = "temp", identifier: str = "anon") -> str:
    """
    Saves an uploaded file to the server and returns the relative path.
    Enforces strict security and follows the new naming convention.
    """
    file_extension = Path(file.filename).suffix.lower()
    
    # 1. Reject disallowed extensions completely
    if file_extension not in ALLOWED_EXTENSIONS:
        raise HTTPException(status_code=400, detail=f"File extension {file_extension} not allowed for security reasons.")
    
    # 2. Reject disallowed MIME types completely
    if file.content_type not in ALLOWED_MIMETYPES:
        raise HTTPException(status_code=400, detail=f"MIME type {file.content_type} not allowed for security reasons.")
    
    # 3. Determine target directory based on category
    subfolder = CATEGORY_MAP.get(category, "temp")
    target_dir = UPLOAD_DIR / subfolder
    os.makedirs(target_dir, exist_ok=True)

    # 4. Generate a descriptive filename: {category}_{identifier}_{timestamp}{ext}
    import time
    timestamp = int(time.time())
    # Clean original filename (remove extension and keep only alphanumeric/underscores)
    import re
    ...
    # Return the relative path for database storage (e.g., 'data/trainees/ids/...')
