@echo off
TITLE NTA User Portal
SETLOCAL EnableDelayedExpansion

REM Change to the directory of this batch file
cd /d "%~dp0"

REM 1. Kill any process on port 7771
echo [PRE-FLIGHT] Checking for existing processes on port 7771...
for /f "tokens=5" %%a in ('netstat -aon ^| findstr :7771') do (
...
echo [STARTING] Starting User Portal Backend (Port 7771)...
echo [INFO] If the window closes immediately, check the output above for errors.
"backend\venv\Scripts\python.exe" -m uvicorn backend.main:app --port 7771 --host 127.0.0.1 --http h11 --loop asyncio
if %errorlevel% neq 0 (
    echo.
