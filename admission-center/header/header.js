(function () {
  document.addEventListener("DOMContentLoaded", function () {
    ...
    container.innerHTML =
      ...
      '<a href="candidates.html" class="nta-header__chip" data-page="candidates" id="headerUsersChip" style="display:none">' +
    ...
    var isAdmissionManager = session.role === "admission_manager";
    var isEditor = session.role === "editor";
    var isLoggedIn = isTrainee || isAdmissionManager || isEditor;
    ...
    if (isAdmissionManager) {
      if (homeChip) {
        homeChip.href = "candidates.html";
        homeChip.setAttribute("data-page", "candidates");
        var s = homeChip.querySelector("span");
        if (s) s.textContent = "المرشحين";
      }
      if (usersChip) usersChip.style.display = "inline-flex";
      if (sectionLabel) sectionLabel.textContent = "القبول";
    } else if (isEditor) {
      if (homeChip) {
