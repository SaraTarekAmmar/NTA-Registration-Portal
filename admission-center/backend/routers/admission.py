...
router = APIRouter(prefix="/api/admission", tags=["Admin"])

@router.post("/upload")
async def upload_admin_file(
    file: UploadFile = File(...),
    folder: str = "reviews",
    staff: dict = Depends(get_admission_manager_user)
):
    """
...
@router.get("/courses")
async def get_admin_courses(current_user: dict = Depends(get_admission_manager_user)):
...
async def get_trainees(stage: Optional[int] = Query(None), role: Optional[str] = Query(None), course_id: Optional[int] = Query(None), staff: dict = Depends(get_admission_manager_user)):
    db = get_db_connection()
...
async def submit_review(review: StageReviewCreate, admin: dict = Depends(get_reviewer_user)):
    ...
    try:
        ...
        # Waitlist (interview stages): score + recommendation saved, candidate held.
        is_waitlist = (
        ...
        if review.result.lower() == 'active':
            if is_waitlist:
                ...
                # Fetch trainee info for the email (include role to send correct portal link)
                cursor.execute("SELECT full_name_ar, email, gender, role FROM users WHERE id = %s", (review.trainee_id,))
                ...
                if user:
                    ...
                    email = user[1]
                    gender = user[2]
                    user_role = user[3] if len(user) > 3 else "trainee"
                    ...
                    # Send Email with role-appropriate portal link (trainee→7771, trainer→7772)
                    send_credential_email(email, full_name, raw_password, gender, role=user_role)
...
async def get_reviews(trainee_id: int, admin: dict = Depends(get_admission_manager_user)):
    db = get_db_connection()
    cursor = db.cursor(dictionary=True, buffered=True)
    try:
...
async def get_trainee_profile(trainee_id: int, staff: dict = Depends(get_admission_manager_user)):
    db = get_db_connection()
    cursor = db.cursor(dictionary=True, buffered=True)
    try:
...
async def get_ai_verification(trainee_id: int, staff: dict = Depends(get_admission_manager_user)):
    db = get_db_connection()
    cursor = db.cursor(dictionary=True, buffered=True)
    try:
...
async def get_cv_matching_result(trainee_id: int, course_id: int, staff: dict = Depends(get_admission_manager_user)):
    db = get_db_connection()
    cursor = db.cursor(dictionary=True, buffered=True)
    try:
...
async def get_admin_attendance(course_id: int, current_user: dict = Depends(get_admission_manager_user)):
    db = get_db_connection()
    cursor = db.cursor(dictionary=True)
    try:
            ...
            for s in sessions:
                ...
                if 'ENTER' in log_entry:
                    ...
                    st["attendance"][sid] = {
                        ...
                        "entry_photo": f"/api/admission/attendance/photo/{st['nationalId']}/{sid}/ENTER",
                    ...
                    if exit_log:
                        ...
                        st["attendance"][sid]["exit_photo"] = f"/api/admission/attendance/photo/{st['nationalId']}/{sid}/LEAVE"
                elif perm_entry:
                    st["attendance"][sid] = {
                        "status": "excused",
...
async def get_trainee_analytics(trainee_id: int, course_id: int, current_user: dict = Depends(get_admission_manager_user)):
    db = get_db_connection()
    cursor = db.cursor(dictionary=True)
    try:
        ...
        for s in sessions:
            ...
            if "ENTER" in l_entry:
                ...
                sess_res.update({
                    ...
                    "entry_photo": f"/api/admission/attendance/photo/{nid}/{sid}/ENTER"
                })
                if leave:
                    sess_res.update({
                        "exit_time": leave["recorded_at"].strftime("%I:%M %p").replace("AM", "ص").replace("PM", "م"),
                        "exit_photo": f"/api/admission/attendance/photo/{nid}/{sid}/LEAVE"
                    })
            elif p_entry:
                sess_res.update({
                    "status": "excused",
