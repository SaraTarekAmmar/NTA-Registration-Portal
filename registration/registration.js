(function () {
    ...
    const API_PREFIX = (window.location.protocol.startsWith('http')) 
        ? '' 
        : 'http://127.0.0.1:7771';  // Trainee portal port
    const MAX_IDENTITY_DOCUMENT_FILES = 3;

    // Trainee portal: role is always 'trainee' — trainer registration is on the trainer portal
    const urlParams = new URLSearchParams(window.location.search);
    const registrationRole = 'trainee'; // Hard-coded for trainee portal
    window.registrationRole = registrationRole;

    const previewStepsParam = urlParams.get('preview_steps');
    const previewSteps = previewStepsParam ? previewStepsParam.split(',').map(Number).filter(n => !isNaN(n) && n > 0) : null;
    ...
    function validateCurrentStep(options = {}) {
        ...
        if (currentStep === 2 && dobInput && dobInput.value) {
        ...
        if (currentStep === 6 && nidInput && nidInput.value.trim().length === 14 && dobInput && dobInput.value && isValid) {
            const birth = new Date(dobInput.value);
        ...
        if (currentStep === 10 && dobInput && dobInput.value) {
        ...
        if (currentStep === 10 || currentStep === 11 || currentStep === 12) {
        ...
        if (currentStep >= 23 && currentStep <= 29) {
            const stepEl7 = stepEl;
            if (stepEl7) {
    ...
    function updateUI() {
        ...
        let progress = 0;
        if (previewSteps) {
            const idx = previewSteps.indexOf(currentStep);
            progress = (idx / Math.max(1, previewSteps.length - 1)) * 100;
        } else {
            progress = ((currentStep - 1) / (TOTAL_STEPS - 1)) * 100;
        }
        ...
        btnPrev.disabled = previewSteps ? (previewSteps.indexOf(currentStep) === 0) : (currentStep === 1);

        const isLastStep = previewSteps ? (currentStep === previewSteps[previewSteps.length - 1]) : (currentStep === TOTAL_STEPS);
        if (isLastStep) {
    ...
    function goNext() {
        if (previewSteps || validateCurrentStep()) {
            let nextStep = currentStep + 1;
            if (previewSteps) {
                const idx = previewSteps.indexOf(currentStep);
                if (idx !== -1 && idx < previewSteps.length - 1) {
                    nextStep = previewSteps[idx + 1];
                } else {
                    nextStep = currentStep;
                }
            }
            let canAdvance = previewSteps ? (nextStep !== currentStep) : (nextStep > currentStep && nextStep <= TOTAL_STEPS);
            if (canAdvance) {
    ...
    function goPrev() {
        ...
        if (previewSteps) {
            const idx = previewSteps.indexOf(currentStep);
            if (idx > 0) {
                prevStep = previewSteps[idx - 1];
            } else {
                prevStep = currentStep;
            }
        }
        let canGoBack = previewSteps ? (prevStep !== currentStep) : (prevStep < currentStep && prevStep >= 1);
        if (canGoBack) {
            saveFormState();
    ...
    async function loadLookups() {
        try {
            const [
                ...
                fetch(API_PREFIX + '/api/registration/lookups/countries').then(res => res.json()),
                fetch(API_PREFIX + '/api/registration/lookups/interests').then(res => res.json()),
                fetch(API_PREFIX + '/api/registration/lookups/languages').then(res => res.json()),
                fetch(API_PREFIX + '/api/registration/lookups/military-status').then(res => res.json()),
                fetch(API_PREFIX + '/api/registration/lookups/identity-doc-types').then(res => res.json()),
                fetch(API_PREFIX + '/api/registration/lookups/degree-levels').then(res => res.json()),
                fetch(API_PREFIX + '/api/registration/lookups/grades').then(res => res.json()),
                fetch(API_PREFIX + '/api/registration/lookups/ministries').then(res => res.json()),
                fetch(API_PREFIX + '/api/registration/lookups/job-titles').then(res => res.json()),
                fetch(API_PREFIX + '/api/registration/lookups/marital-status').then(res => res.json()),
                fetch(API_PREFIX + '/api/registration/lookups/monthly-income').then(res => res.json())
            ]);
    ...
    async function updateStates() {
        ...
        try {
            const states = await fetch(API_PREFIX + `/api/registration/lookups/states/${countryId}`).then(res => res.json());
            stateSelect.innerHTML = '<option value="">اختر المحافظة / الولاية</option>';
    ...
    async function updateUniversitiesForCountry(countryId, targetSelects) {
        if (!countryId) return;
        try {
            const univs = await fetch(API_PREFIX + `/api/registration/lookups/universities/${countryId}`).then(res => res.json());
    ...
    function setupStepThreeEducationalLogic() {
        ...
        function bindMainEducationCard(card) {
            ...
            async function updateInstitutionCollegeMode() {
                ...
                if (/^\d+$/.test(val)) {
                    try {
                        const colleges = await fetch(API_PREFIX + `/api/registration/lookups/universities/${val}/colleges`).then(res => res.json());
    ...
    function setupStepFourEmploymentLogic() {
        ...
        async function fillMinistrySub(ministrySel, subSel) {
            ...
            if (/^\d+$/.test(m)) {
                try {
                    const authorities = await fetch(API_PREFIX + `/api/registration/lookups/ministries/${m}/authorities`).then(res => res.json());
    ...
    async function initSkills() {
        ...
        try {
            const response = await fetch(API_PREFIX + '/api/registration/lookups/skills');
            if (response.ok) {
            ...
            const langRes = await fetch(API_PREFIX + '/api/registration/lookups/languages');
            if (langRes.ok) {
                languagesList = await langRes.json();
            }

            const profRes = await fetch(API_PREFIX + '/api/registration/lookups/proficiencies');
            if (profRes.ok) {
    ...
    function showSkillsError() {
        ...
        warning.innerHTML = `<strong>تنبيه (Warning):</strong> تعذر الاتصال بخادم المهارات. يرجى التأكد من تشغيل <code>RUN_REGISTRATION.bat</code> وفتح الرابط <a href="http://localhost:7775/index.html">/index.html</a>.`;
    ...
    async function uploadFile(file, folder = 'general', inputElement = null, allowedMimeTypes = null) {
        ...
        try {
            const response = await fetch(API_PREFIX + `/api/registration/upload?folder=${folder}`, {
                method: 'POST',
                body: formData
            });
            if (!response.ok) {
    ...
    form.addEventListener('submit', async function (e) {
        ...
        // Actual API Call
        fetch(API_PREFIX + '/api/registration/register', {
            method: 'POST',
            headers: { 
    ...
    restoreFormState();
    if (previewSteps && !previewSteps.includes(currentStep)) {
        currentStep = previewSteps.length > 0 ? previewSteps[0] : 1;
    }
